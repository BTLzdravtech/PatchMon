...
4. Add a migration in `internal/migrate/migrations/` with both `.up.sql` and `.down.sql` files. **Check the directory for the latest migration number before choosing yours** - duplicate numbers crash startup.
   **Fork-only changes (BTLzdravtech):** never add to `migrations/`. Put them in `internal/migrate/migrations_fork/` instead, numbered from `000001`, so upstream merges cannot collide. See the README in that directory.
5. Run `make check`.
